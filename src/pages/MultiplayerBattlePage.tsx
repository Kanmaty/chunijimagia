...
import { Zap, CircleDot, Flame, Shield, LoaderCircle } from 'lucide-react';
...
export const MultiplayerBattlePage: React.FC = () => {
    const { roomId } = useParams<{ roomId: string }>();
    const navigate = useNavigate();
    const { roomData, myUid, setRoomId } = useRoom();

    useEffect(() => {
        if (roomId) setRoomId(roomId);
    }, [roomId, setRoomId]);

    const game = useMultiplayerGame(roomId || null, myUid, roomData);
    ...
    }, [currentTurn, roomData?.status]);

    if (!roomData || roomData.status !== 'playing' || !myCharacter || !enemyCharacter) {
        return (
            <div className="min-h-screen bg-stone-950 flex flex-col items-center justify-center text-white p-4 text-center space-y-4">
                <LoaderCircle className="w-12 h-12 text-cyan-400 animate-spin" />
                <h2 className="text-xl font-bold">対戦準備中...</h2>
                <p className="text-slate-400 text-sm">相手のデッキ作成完了を待機しています</p>
                <button onClick={() => navigate('/')} className="mt-8 px-4 py-2 border border-stone-600 rounded-lg text-sm text-stone-400 hover:bg-stone-800 transition-colors">
                    トップに戻る
                </button>
            </div>
        );
                ...
                >
                    ...
                    <div className="flex-1 flex flex-col items-start justify-start px-2 pt-1 pointer-events-auto overflow-y-auto scrollbar-hide">
                        <div className="flex flex-col gap-1 items-start">
                            {enemyCharacter.equippedSkills.filter(s => enemySkills.includes(s.id)).map((skill) => (
                                <button
                                    key={`enemy-${skill.id}`}
                        ...
                        <div className="flex items-center gap-3 bg-black/60 p-2 rounded-xl backdrop-blur border border-white/10 shadow-xl pointer-events-auto">
                            ...
                            <div>
                                ...
                                <div className="mt-2 flex gap-1">
                                    {myCharacter.equippedSkills.filter(s => mySkills.includes(s.id)).map((skill) => {
                ...
                {gameResult && (
                    <div className="fixed inset-0 z-[300] flex flex-col items-center justify-center bg-black/90 backdrop-blur-md">
                        <div className={`text-4xl font-black italic tracking-widest mb-6 ${gameResult === 'win'
                            ...
                            : gameResult === 'lose' ? 'text-red-500 drop-shadow-[0_0_40px_rgba(239,68,68,0.8)]'
                                : 'text-yellow-500 drop-shadow-[0_0_40px_rgba(234,179,8,0.8)]'
                            }`}>
